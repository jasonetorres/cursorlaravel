<nav x-data="{ open: false, darkMode: localStorage.getItem('darkMode') === 'dark' }" 
     class="border-b border-gray-100 bg-gradient-to-r from-blue-400 to-purple-500 dark:from-gray-800 dark:to-gray-900 dark:border-gray-700">
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden space-x-4 sm:flex sm:items-center sm:ms-6">
                <a href="{{ route('events.create') }}" class="px-3 py-2 text-sm font-medium text-white transition duration-150 ease-in-out bg-blue-600 rounded-md hover:bg-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                    {{ __("Create Event") }}
                </a>
                <a href="{{ route('events.index') }}" class="px-3 py-2 text-sm font-medium text-white transition duration-150 ease-in-out bg-blue-600 rounded-md hover:bg-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                    {{ __("View Events") }}
                </a>
                <a href="{{ route('settings.index') }}" class="px-3 py-2 text-sm font-medium text-white transition duration-150 ease-in-out bg-purple-600 rounded-md hover:bg-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50">
                    {{ __("Settings") }}
                </a>

                <!-- Dark Mode Toggle -->
                <button @click="darkMode = !darkMode; localStorage.setItem('darkMode', darkMode ? 'dark' : 'light')"
                        x-init="$watch('darkMode', val => document.documentElement.classList.toggle('dark', val))"
                        class="p-1 text-white transition duration-150 ease-in-out bg-purple-600 rounded-md hover:bg-purple-300 focus:outline-none focus:ring-1 focus:ring-purple-500 focus:ring-opacity-50">
